  ...
  let list = VT.questions.filter(item => {
    ...
    if (q) {
      const hay = (item.stem + ' ' + (item.subs || []).join(' ') + ' ' +
                   (item.options || []).join(' ') + ' ' + (item.subtopic || '') + ' ' +
                   (item.passageText || '')).toLowerCase();
...
function questionCard(item) {
  ...
  const topicTag = `<span class="tag topic">${esc(item.subject)}${item.subtopic ? ' · ' + esc(item.subtopic) : ''}</span>`;
  ...
  return `<article class="qcard${prog ? ' attempted' : ''}" data-id="${esc(item.id)}">
    ...
    ${item.stem ? `<div class="qstem">${esc(item.stem)}</div>` : ''}
    ${subsHtml}
    ${optsHtml}
...
/* Passage block shown once above a cluster of questions that share a passageId. */
...
/* Keep questions that share a passageId contiguous, in first-seen order. */
function coalesceClusters(list) {
  const idx = new Map(); const groups = [];
  list.forEach(item => {
    const pid = item.passageId;
    if (pid) {
      if (idx.has(pid)) { idx.get(pid).push(item); }
      else { const arr = [item]; idx.set(pid, arr); groups.push(arr); }
...
function render() {
  ...
  const list = coalesceClusters(getFiltered());
  ...
  let lastPid = '';
  shown.forEach(item => {
    if (item.passageId && item.passageId !== lastPid) { html += passageBlock(item); }
    lastPid = item.passageId || '';
    html += questionCard(item);
  });
...
function renderSubjectHead(name) {
  ...
  if (name) {
    ...
    document.title = `${name} - VicThree Defence CDS English PYQ`;
    el.innerHTML =
      `<a class="back" href="index.html">← All sections</a>
       <h2 class="sh-title">${esc(name)} <span class="sh-count">${s ? s.count : ''} Qs</span></h2>
       <div class="quiz-cta"><a class="btn quizbtn" href="quiz.html?section=${encodeURIComponent(name)}">🎲 Random 50 Quiz - ${esc(name)}</a></div>`;
  } else {
    el.innerHTML =
      `<a class="back" href="index.html">← Home</a>
       <h2 class="sh-title">All PYQs</h2>
       <div class="quiz-cta"><a class="btn quizbtn" href="quiz.html">🎲 Random 50 Quiz - all sections</a></div>`;
  }
}
...
async function init() {
  ...
  fillSelect(els.subject, subs, 'All sections');
