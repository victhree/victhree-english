/* VicThree Defence CDS PYQ - welcome popup / lead capture.
   Shows a one-time gate asking for name, phone and email, then posts the
   details to a Google Form (linked to a Google Sheet, set in config.js).
   Once submitted, it never shows again for that visitor (localStorage). */
(function () {
  "use strict";

  var KEY = "vteng_lead_done";
  var CFG = window.VTENG_CONFIG || {};
  ...
  var GREETINGS = [
    ["Welcome back, {name}.", "Every paper you solve sharpens the officer in you."],
    ["Good to see you, {name}.", "Vocabulary and grammar reward the steady, not the sudden."],
    ["{name}, let us put in the reps.", "The question patterns repeat; make sure your practice does too."],
    ["Sharp and steady, {name}.", "Master the past papers and the English section holds no surprises."],
    ["Back at it, {name}.", "A few honest questions today beat a hundred someday."],
    ["{name}, the cutoff rewards the prepared.", "Read, attempt, review, repeat."],
    ["Keep the momentum, {name}.", "Clear English is a habit built in quiet hours like these."],
    ["Rise a little sharper, {name}.", "That is what the CDS English paper really measures."]
  ];

  function firstName() {
    var raw = "";
    try { raw = (JSON.parse(localStorage.getItem("vteng_lead_data") || "{}").name) || ""; } catch (e) {}
    raw = raw.trim();
    if (!raw) return "";
    var f = raw.split(/\s+/)[0];
  ...
  function renderGreeting() {
    var anchor = document.querySelector("#hub"); // homepage only
    if (!anchor || document.querySelector(".greet")) return;
    var name = firstName();
    if (!name) return;

    var i = 0;
    try { i = parseInt(localStorage.getItem("vteng_greet_i") || "0", 10) || 0; } catch (e) {}
    var msg = GREETINGS[i % GREETINGS.length];
    try { localStorage.setItem("vteng_greet_i", String((i + 1) % GREETINGS.length)); } catch (e) {}
  ...
  function build() {
    ...
    card.innerHTML =
      '<div class="lead-banner"><img src="assets/banner.jpg" alt="VicThree Defence, by Anmol Sharma"></div>' +
      '<div class="lead-body">' +
  ...
  function show() {
    ...
    form.addEventListener("submit", function (ev) {
      ...
      try { localStorage.setItem("vteng_lead_data", JSON.stringify(data)); } catch (e) {}
